...
const handleNoteDelete = (e) => {
  ...
  const note = JSON.parse(noteEl.parentElement.getAttribute(`data-note`));
...
const renderNoteList = async (notes) => {
  ...
  const createLi = (title, delBtn = true) => {
    ...
    spanEl.setAttribute('onclick', `handleNoteView(event)`);
  ...
  jsonNotes.forEach((note) => {
    ...
    const li = createLi(title);
    li.setAttribute('data-note',`{"title":"${title}","text":"${text}","id":"${id}"}`);
